...
export default class PatchNotesApi {
  ...
  static getPatchNotesStats(): IPatchNotesStats {
    ...
    return {
      champions: champions.reduce(
        (obj, id) => ({
          ...obj,
          [id]: this.getPatchNotesChampionStats(id, keystones, buildStats),
        }),
        {}
      ),
      runes: runes.reduce(
  ...
  static getPatchNotesChampionStats(
    ...
    buildStats = buildStats
      .filter(
        (b) =>
          b.championId == championId && !BuildStats.isSmallRune(b, keystones)
      )
      .filter(
        (s) =>
          getPickrateIncrease(s, s.totalMatches, s.previousTotalMatches) > 0.05
      )
      .slice(0, 5);
