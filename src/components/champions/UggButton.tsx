...
export default function UggButton({ champion }) {
  ...
  return (
    <div className="flex bg-ugg-dark rounded-md px-3 py-2 text-white shadow focus:outline-none">
      ...
      <Menu as="div" className="relative flex items-center ml-3">
        ...
        <Menu.Items className="flex flex-col align-center absolute -right-3 w-48 top-9 origin-top-right rounded-md shadow-lg bg-ugg-dark py-2 z-10">
          {MENU_ITEMS.map((i) => (
            <Menu.Item key={i.title}>
              {({ active }) => (
                <a
                  href={i.link(name)}
                  className={`py-1 px-3 ${active && "bg-ugg"}`}
                  target="_blank"
                >
                  {i.title}
                </a>
